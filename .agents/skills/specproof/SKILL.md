...
name: specproof
description: >
  Single entry point to introduce and drive specproof in a repository. Detects the
  ...
  "specproof を導入して", "introduce / set up specproof", or asks to start or continue
  the BDD flow and is not sure which skill to run.
---

# specproof オーケストレータ（単一の入口）

このスキルは specproof の**唯一の入口**。利用者は方向ラベル（impl→feature 等）を覚える必要はない。
オーケストレータが状況を検出し、内部ムーブメントを順に駆動し、**判断が要る所で停止**して
ハンドオフレポートを出す。方向ラベルの不変条件は methodology が内部で担保する。

> 内部ムーブメント（このスキルが駆動する。単体起動も可）:
> `specproof-setup`（検出・scaffold）/ `specproof-bootstrap`（impl→feature ドラフト）/
> `specproof-new-feature`（spec→feature）/ `specproof-implement`（feature→impl）/ `specproof-sync`（drift→feature）。
> drift の **検知**はスキルではなく決定論 CLI `{{config:commands.traceabilityCheck}}`（AI 不使用）。

## 守る不変条件（割ってはいけない）

- **実装は blessed feature にのみ**。`# specproof: draft` マーカーが残る feature は未査読ドラフト＝
  実装しない（同語反復ファイアウォール）。`{{config:commands.traceabilityCheck}} --strict` と
  `specproof-implement` の着手前検査が機械的に止める。
...
1. リポルートの `specproof.config.yaml` を探す。
   - **既にある** → 検証モード。`{{config:commands.traceabilityCheck}}` で drift を確認し、
     必要なら §3 / §5 のサイクルへ。
   - **無い** → `specproof-setup` ムーブメントを駆動して導入する: `npx -y @pound79/specproof detect --json`
     で adapter を検出（曖昧なら「ブラウザで操作する Web アプリですか？」と 1 問だけ確認）→
     `npx -y @pound79/specproof init --adapter <adapter> --dir <dir>` で scaffold →
     ...
     **重い副作用（npm install 等）は実行せず案内のみ**。
     - **detect が落ちても推測着手しない**（出力を見ずに adapter を断定するのは禁止）。エラー別に対処:
       - **`ENOVERSIONS`（No versions available）** → registry 問題ではなく **`min-release-age` cooldown**
         ...
         (b) `npx -y --min-release-age=0 @pound79/specproof detect --json`（**npm option は package 名より前**。
         後ろに置くと specproof に渡り `Unknown command` になる）/ (c) `.npmrc` の `min-release-age` を一時
       ...
       - **`E404` / 真の private・scoped registry** → public registry を `env` プレフィックスで明示して再実行
         ...
         `env 'npm_config_registry=https://registry.npmjs.org/' 'npm_config_@pound79:registry=https://registry.npmjs.org/' npx -y @pound79/specproof detect --json`
...
- **不可逆な作業**（bless＝featuresDir への移動・`specproof-implement` による実装）は**ドメイン毎の
  明示ゲート**。査読なしに権威を刻ませない。

## 3. brownfield フロー

1. ドメインごとに `specproof-bootstrap` を駆動し、**ドラフトを scratch に一括生成**
   （`# specproof: draft` マーカー入り。featuresDir は上書きしない）。
...
4. 人間がドラフトを査読・意図を刻印し（境界値・エラー経路・ロール分岐・rationale リンク、
   **マーカー行を削除**）featuresDir へ移したドメインだけ、`specproof-implement` を**1 つずつゲート**で駆動。

## 4. greenfield フロー

1. `specproof-setup` で scaffold（adapter 低信頼になりがち → 1 問確認）。
2. **env セットアップを能動支援**（dev サーバ起動コマンド検出→baseUrl 配線、`.env.example` 生成、
   認証 setup 雛形）。
3. 人間が挙げた**観測可能な振る舞い 1 本**を、spec 見出し+WHY の骨子 → `specproof-new-feature`(RED) →
   `specproof-implement` → green まで**通して駆動**（ワークエド・サンプル）。
...
- **(b) spec 骨子抽出（既定）**: AI は「見出し＋観測可能トピックの骨子」までを impl から起こし、
  中身の権威（なぜ・正しさ）は人間が確定 → `specproof-new-feature`。`impl→骨子→人間が著した spec`。
...
- drift あり → `specproof-sync <link-id>`（決定表に従い feature を追従、人間ゲートで bless）。
- 仕様の新セクション → `specproof-new-feature`。RED にした feature → `specproof-implement`。
