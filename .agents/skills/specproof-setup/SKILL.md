---
name: specproof-setup
description: >
  Internal movement of the /specproof orchestrator: detect the framework
  (Flutter / Playwright web), scaffold config + templates via `specproof init`,
  then tailor specproof.config.yaml to the repo. Prefer running /specproof (the
...
# BDD Setup (リポジトリに specproof を導入する)

> 内部ムーブメント — 通常は `/specproof`（オーケストレータ）が駆動する。単体起動も可。

対象リポジトリのフレームワーク (Flutter / Playwright web) を自動検出し、
`specproof init` でテンプレートを scaffold したうえで、
`specproof.config.yaml` をリポジトリの実態に合わせて調整する。

## 重要な前提: specproof はバックエンド実装言語に依存しない

specproof の E2E テストは **UI 層**（ブラウザ画面 or モバイル画面）を操作対象とする。
...
1. リポルートに `specproof.config.yaml` が**既に存在する場合**:
   - 「specproof は導入済みです」と伝え、設定の検証モードに切り替える。
   - `npx -y -p @pound79/specproof-traceability specproof-check --json` で drift 確認を提案。
   - `/specproof`（オーケストレータ）または `specproof-check`（drift 検知 CLI）を案内して終了。
...
```bash
npx -y @pound79/specproof detect --json
```
...
>   後ろだと specproof に渡って `Unknown command`）/ (c) `.npmrc` の `min-release-age` を一時コメントアウト。
...
```bash
npx -y @pound79/specproof init --adapter {{adapter}} --dir {{dir}}
```

出力を確認し、scaffold されたファイル一覧をユーザーに報告する。
既存ファイルが skip された場合はその旨を伝え、`--force` の使用について案内する。

## 4. specproof.config.yaml の調整（このスキルの付加価値）

生成された `specproof.config.yaml` をリポの実態に合わせて編集する。
各変更に理由を1行添えて報告する。
...
- 既存の実装ファイルがあるドメイン → `/specproof-bootstrap` を案内
- 新規機能の仕様から始める場合 → `/specproof-new-feature` を案内
- スキルが未導入の場合 → エージェントに応じたインストールを案内:
  - **Claude Code**: `/plugin marketplace add Pound79/specproof` → `/plugin install specproof@specproof`
  - **Codex / その他**: `npx @pound79/specproof setup-agent codex` で `.agents/skills/` にスキルを配置
  - **手動**: `plugins/specproof/skills/` 以下の SKILL.md をエージェントのスキルディレクトリにコピー

## Safety rules

- 既存の `specproof.config.yaml` を無断で上書きしない。
