...
リポルートの `bdd-kit.config.yaml` を読み、以降に登場するすべての `{{config:...}}` トークンを
そのファイルの対応フィールド値で置き換えてから作業を開始すること。
このファイルが存在しない場合は **STOP し、ユーザーに
`bdd-kit init --adapter <framework>` の実行を依頼する**。
...
| skill               | 方向                     | いつ使う                                            |
| ------------------- | ------------------------ | --------------------------------------------------- |
| `bdd-new-feature`   | spec → feature           | 仕様セクションがあり E2E 未整備の**新規**機能       |
| `bdd-sync`          | spec/impl diff → feature | drift 検知後の同期                                  |
| **`bdd-bootstrap`** | **impl → feature**       | **既存実装はあるが feature が薄い**ドメインの底上げ |
...
実装変更の feature 追従は再生成ではなく `{{config:commands.traceabilityCheck}}`（検知・CLI）→ `/bdd-sync`（差分反映・人間
...
4. `../bdd-sync/prompts/system.md`（または `{{config:layout.idiomGuide}}` が指すガイド）の生成ガイドに従い、
...
5. **ドラフトはスクラッチに出す**（例 `{{config:layout.scratchDir}}/<domain>.feature`）。
   各ドラフトの先頭付近（`# language:` 行の直後）に **`# bdd-kit: draft` マーカー行**
   ...
   `unreviewed-draft` として失敗させ、`bdd-implement` も着手を拒否する（同語反復ループ防止）。
...
7. **人間の査読ゲート（必須）**: ユーザーがドラフトを読み、意図を刻印（編集・取捨選択し、
   `# bdd-kit: draft` マーカー行を削除）してから `{{config:layout.featuresDir}}/` へ移す。移したら新規 step 句のうち既存 steps で賄えないものを
   `bdd-new-feature` の Step 4 と同様に stub 化する（既存 `{{config:layout.stepsDir}}/*{{config:layout.stepFileSuffix}}` への追加を優先し、
...
8. 既存 feature の拡張なら bless:
   ...
   ```
   新規リンクなら `{{config:layout.manifest}}` に追記（`bdd-new-feature` の手順に準拠）。
...
（bdd-bootstrap パイロットで実証済み）。AI が許されるのは「topic の骨子出し」までで、
...
- **ドラフトには `# bdd-kit: draft` マーカーを必ず入れる**。featuresDir へ移すのは人間が
...
- `{{config:layout.testRunnerConfig}}` の project 構成（`{{config:projects}}`）に
  合わないタグを付けない（`bdd-new-feature` の Safety rules に準拠）。
