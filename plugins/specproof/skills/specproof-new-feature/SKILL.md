---
name: specproof-new-feature
description: Bootstrap a new BDD feature from a spec document section. Creates a Gherkin .feature file (in the language specified by specproof.config.yaml), stub step definitions, and a new traceability manifest link, then validates with the configured bdd generation tool. Use when a new spec section exists but has no E2E coverage yet. Driven entirely by specproof.config.yaml — no repo-specific values are hardcoded in this skill.
---
...
```
specproof.config.yaml の読み込み先: <repo-root>/specproof.config.yaml
```
...
`specproof.config.yaml` がリポルートに存在しない場合は**ここで停止し**、
ユーザーに次のコマンドの実行を促すこと:

```bash
specproof init --adapter <framework>
```
...
2. このセクションを実装しているファイル (web ページ / lambda 等) を特定する。
   ...
   (エージェントのユーザー確認ツールを使う)。
3. `../specproof-sync/prompts/system.md`（存在する場合）または `{{config:layout.idiomGuide}}`（設定されている場合）の生成ガイドに従い
