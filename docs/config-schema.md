# bdd-kit.config.yaml スキーマリファレンス

consumer リポのルートに置く 1 枚の設定ファイル。bdd-* skill は実行時にこれを読み、
プロンプト内の `{{placeholder}}` を実値へ解決する。これにより skill 本体（SKILL.md）は
リポ非依存になり、Claude Code plugin として無改変配布できる。

正準の worked example は `templates/playwright/bdd-kit.config.yaml`。
...
- traceability エンジン（`@pound79/bdd-traceability`）は `layout` のパスのみを受け取り、
  framework を一切知らない。
...
| キー | 要否 | playwright 例 |
|---|---|---|
| `generate` | required | `npm -w e2e run bddgen` |
| `typecheck` | required | `npm -w e2e run typecheck` |
| `lint` | required | `npm run lint` |
| `smoke` | required | `npm -w e2e run test:smoke` |
| `install` | optional<sup>+</sup> | `npm ci` |
| `traceabilityUpdate` / `traceabilityCheck` / `traceabilityList` / `traceabilityStats` | optional | `npx -y -p @pound79/bdd-traceability bdd-traceability-*` |
...
| `implGlobs` | required | traceability 登録 impl の glob リスト |
| `scratchDir` / `testRunnerConfig` / `e2eReadme` / `idiomGuide` | optional | 補助参照 |
...
| ブロック | 要否 | 要点 |
|---|---|---|
| `tags` | required（slow/generate/admin/user） | Gherkin タグ正準名。sf/fixme/skip は optional。`fixme` / `skip` は省略時 `@fixme` / `@skip`。traceability エンジンの理由コメント必須タグ（`bdd-traceability-check`）と done gate（`bdd-traceability-stats --strict` の @fixme=0 判定）にも使われるため、リネーム時はこの値で集計・lint される。`@` は省略可（自動補完）。 |
...
| `implement`<sup>+</sup> | optional | `blastRadiusGlobs` = bdd-implement の編集許可スコープ（implGlobs とは別概念）。 |
| `git`<sup>+</sup> | optional | `commitScope` / `branchPrefix`。bdd-sync 自動コミット用。 |
...
- **`BDD_KIT_ENV` 環境変数**でアクティブプロファイルを選択する。
- **`{{config:auth.provider}}` トークンはアクティブ環境プロファイルの `auth.provider` に解決される。**

### 選択ルール

1. `BDD_KIT_ENV` が設定されている → 一致する `name` のエントリを使用。不一致時はエラー。
2. `BDD_KIT_ENV` が未設定 → `default: true` のエントリ。複数ある場合は先頭。
3. `default: true` もない → `environments[]` の先頭エントリ。

### フィールド

| フィールド | 型 | 要否 | 説明 |
|---|---|---|---|
| `name` | string | required | 環境名（`local` / `dev` / `staging` / `prod` 等）。`BDD_KIT_ENV` の値と照合する。 |
| `default` | boolean | optional | `BDD_KIT_ENV` 未設定時に使うプロファイル。1 つだけ `true` にする。 |
...
```
environments[]  — 実行環境軸（local/dev/staging）  → BDD_KIT_ENV で選択
...
3. `implement.blastRadiusGlobs` に `packages/web/src/components/exampleApp/**` — implGlobs から
   漏れていた bdd-implement の編集スコープ（gap/correction）。
...
5. `projects[].features` — guest が `auth.feature` のみ実行する事実は tag だけでは表現不可
   だった（correction）。
6. `git.commitScope` / `git.branchPrefix` — `feat(e2e):` / `feat/bdd-sync-<date>` が hardcode（gap）。
